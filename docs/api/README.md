...
- endpoint `component-quality` pokazuje dla konkretnego urządzenia jakość każdego zainstalowanego komponentu, w tym `component_qc_passed`, otwarte krytyczne NCR, `blocks_shipment`, status `PASS`, `QC_NOT_PASSED` albo `CRITICAL_NCR_OPEN`, a na poziomie urządzenia także `primary_quality_status`, `recommended_action` i `stale_bucket`
- endpoint kolejkowy `GET /api/component-quality` zbiera taki sam widok dla wielu urządzeń, dodaje `quality_status_summary`, `variant_code_summary`, `production_status_summary`, `primary_quality_status_summary`, `staleness_summary`, `component_type_summary`, `recommended_action_summary` i wspiera filtry `device_type`, `variant_code`, `production_status`, `component_type`, `quality_status`, `primary_quality_status`, `stale_bucket`, `recommended_action`, `created_after`, `created_before`, `updated_after`, `updated_before`, `only_blocking`, `sort_by`, `sort_desc`, `offset` i `limit`; rekord urządzenia niesie też `device_created_at`, `device_updated_at` i własny `stale_bucket`, a `sort_by` obsługuje dziś `created_at`, `updated_at`, `device_serial_number`, `blocked_components`, `production_status`, `stale_bucket`, `variant_code` i `recommended_action`
