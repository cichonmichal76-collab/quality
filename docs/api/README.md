...
- endpoint kolejkowy `GET /api/shipment-readiness` zwraca listę urządzeń wraz z tym samym kontraktem gotowości, plus liczniki `ready_count` i `blocked_count` oraz agregację `blocking_summary`; wspiera filtry `device_type`, `variant_code`, `blocking_code`, `recommended_action`, `only_blocked`, `only_ready` i `limit`
