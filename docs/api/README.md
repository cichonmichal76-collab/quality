...
- endpoint kolejkowy `GET /api/component-quality` zbiera taki sam widok dla wielu urządzeń, dodaje `quality_status_summary` i wspiera filtry `device_type`, `variant_code`, `production_status`, `quality_status`, `only_blocking`, `sort_by`, `sort_desc`, `offset` i `limit`
