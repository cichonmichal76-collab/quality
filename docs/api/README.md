...
- endpoint `component-quality` pokazuje dla konkretnego urządzenia jakość każdego zainstalowanego komponentu, w tym `component_qc_passed`, otwarte krytyczne NCR, `blocks_shipment` i status `PASS`, `QC_NOT_PASSED` albo `CRITICAL_NCR_OPEN`
- endpoint kolejkowy `GET /api/component-quality` zbiera taki sam widok dla wielu urządzeń, dodaje `quality_status_summary`, `component_type_summary` i wspiera filtry `device_type`, `variant_code`, `production_status`, `component_type`, `quality_status`, `only_blocking`, `sort_by`, `sort_desc`, `offset` i `limit`
