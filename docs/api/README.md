...
- endpoint kolejkowy `GET /api/component-quality` zbiera taki sam widok dla wielu urządzeń, dodaje `quality_status_summary`, `component_type_summary`, `recommended_action_summary` i wspiera filtry `device_type`, `variant_code`, `production_status`, `component_type`, `quality_status`, `recommended_action`, `only_blocking`, `sort_by`, `sort_desc`, `offset` i `limit`
