...
- endpoint kolejkowy `GET /api/component-quality` zbiera taki sam widok dla wielu urządzeń, dodaje `quality_status_summary`, `variant_code_summary`, `production_status_summary`, `primary_quality_status_summary`, `primary_blocking_component_type_summary`, `staleness_summary`, `component_type_summary`, `recommended_action_summary` i wspiera filtry `device_type`, `variant_code`, `production_status`, `component_type`, `quality_status`, `primary_quality_status`, `primary_blocking_component_type`, `primary_blocking_component_serial_number`, `stale_bucket`, `recommended_action`, `passes_component_quality_gate`, `created_after`, `created_before`, `updated_after`, `updated_before`, `only_blocking`, `sort_by`, `sort_desc`, `offset` i `limit`; rekord urządzenia niesie też `device_created_at`, `device_updated_at`, `passes_component_quality_gate`, własny `stale_bucket`, `primary_blocking_component_type` i `primary_blocking_component_serial_number`, a `sort_by` obsługuje dziś `created_at`, `updated_at`, `device_serial_number`, `blocked_components`, `production_status`, `primary_blocking_component_type`, `primary_blocking_component_serial_number`, `stale_bucket`, `variant_code` i `recommended_action`
