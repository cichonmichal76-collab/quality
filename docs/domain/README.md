...
- widok kolejkowy `component-quality` pozwala zebrać te same dane dla wielu urządzeń, agregować komponenty per `quality_status` i sortować urządzenia np. po liczbie blokujących komponentów albo serialu, co pomaga jakości i produkcji szybciej rozbić problemy na `QC_NOT_PASSED` oraz `CRITICAL_NCR_OPEN`
