...
- widok kolejkowy `shipment-readiness` pozwala pobrać zbiorczo urządzenia gotowe i zablokowane do wysyłki, a także ich agregację per `blocking_code`, `primary_blocking_code` i `recommended_action`; pojedynczy i zbiorczy odczyt zwracają też `recommended_action`, co upraszcza budowę dashboardu operacyjnego
