...
- odczyt `component-quality` pokazuje dla konkretnego urządzenia jakość każdego zainstalowanego komponentu, w tym snapshot `component_qc_passed`, otwarte krytyczne NCR i to, czy dany komponent sam blokuje shipment, a dodatkowo wylicza dla całego urządzenia `primary_quality_status` i `recommended_action`
- widok kolejkowy `component-quality` pozwala zebrać te same dane dla wielu urządzeń, agregować komponenty per `quality_status`, a urządzenia per `variant_code`, `production_status`, `primary_quality_status`, `passes_component_quality_gate`, `primary_blocking_component_type`, `blocking_component_type`, `stale_bucket`, `component_type` i `recommended_action`; każdy rekord niesie też `passes_component_quality_gate`, własny `stale_bucket`, `primary_blocking_component_type` i `primary_blocking_component_serial_number`, więc UI może pokazać zarówno werdykt bramki, wiek problemu, jego główne źródło, jak i konkretną część do sprawdzenia bez dodatkowych przeliczeń. Dochodzi też bezpośredni filtr po `passes_component_quality_gate`, `blocking_component_type` i `primary_blocking_component_serial_number`, więc kolejkę można zawęzić do sztuk przechodzących lub blokowanych, do dowolnego typu blokującego komponentu oraz do jednej fizycznej sztuki komponentu. Sortowanie obejmuje już nie tylko liczbę blokujących komponentów, wariant BOM, rekomendowaną akcję, czas utworzenia, czas ostatniej zmiany albo serial, ale też bezpośrednio `passes_component_quality_gate`, `stale_bucket`, `primary_blocking_component_type` i `primary_blocking_component_serial_number`. Dochodzi też filtrowanie po oknach `created_after` / `created_before` i `updated_after` / `updated_before`, co pomaga jakości i produkcji szybciej wyciągać nowe sztuki, świeże zmiany albo zaległości na konkretnym etapie procesu
