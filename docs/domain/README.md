...
- odczyt `component-quality` pokazuje dla konkretnego urządzenia jakość każdego zainstalowanego komponentu, w tym snapshot `component_qc_passed`, otwarte krytyczne NCR i to, czy dany komponent sam blokuje shipment
- widok kolejkowy `component-quality` pozwala zebrać te same dane dla wielu urządzeń, agregować komponenty per `quality_status` i `component_type` oraz sortować urządzenia np. po liczbie blokujących komponentów albo serialu, co pomaga jakości i produkcji szybciej rozbić problemy na `QC_NOT_PASSED`, `CRITICAL_NCR_OPEN` i konkretne typy części
