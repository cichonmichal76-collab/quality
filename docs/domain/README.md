...
- widok kolejkowy `component-quality` pozwala zebrać te same dane dla wielu urządzeń, agregować komponenty per `quality_status`, a urządzenia per `variant_code`, `production_status`, `primary_quality_status`, `primary_blocking_component_type`, `stale_bucket`, `component_type` i `recommended_action`; każdy rekord niesie też własny `stale_bucket` oraz `primary_blocking_component_type`, więc UI może pokazać zarówno wiek problemu, jak i jego główne źródło bez dodatkowych przeliczeń. Sortowanie obejmuje już nie tylko liczbę blokujących komponentów, wariant BOM, rekomendowaną akcję, czas utworzenia, czas ostatniej zmiany albo serial, ale też bezpośrednio `stale_bucket` i `primary_blocking_component_type`. Dochodzi też filtrowanie po oknach `created_after` / `created_before` i `updated_after` / `updated_before`, co pomaga jakości i produkcji szybciej wyciągać nowe sztuki, świeże zmiany albo zaległości na konkretnym etapie procesu
