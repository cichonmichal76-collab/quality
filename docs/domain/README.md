...
- widok kolejkowy `component-quality` pozwala zebrać te same dane dla wielu urządzeń, agregować komponenty per `quality_status`, a urządzenia per `primary_quality_status`, `component_type` i `recommended_action`, oraz sortować urządzenia np. po liczbie blokujących komponentów albo serialu, co pomaga jakości i produkcji szybciej rozbić problemy na `QC_NOT_PASSED`, `CRITICAL_NCR_OPEN`, konkretny główny problem sztuki, typ części i następną akcję operacyjną
