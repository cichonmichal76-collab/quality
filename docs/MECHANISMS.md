...
Status READY_FOR_SHIPMENT jest możliwy tylko wtedy, gdy urządzenie ma wymagane komponenty, nie ma komponentów nadmiarowych ani nieoczekiwanych względem BOM, zainstalowane komponenty zachowują snapshot pozytywnego QC, final test jest PASS i nie ma blokujących NCR ani na urządzeniu, ani na zainstalowanych komponentach.

W obecnym MVP wymagane komponenty są odczytywane z aktywnego BOM zapisanego w tabelach `device_bom_templates` i `device_bom_items`. Backend porównuje ilości wymagane z faktycznie zainstalowanymi `AssemblyLink`, a bazowa migracja dostarcza minimalny BOM dla `ZSS`, wymagający `CONTROL_PCB`. Część walidacji BOM dzieje się już podczas assembly scan, a pierwszy poprawny montaż przypina urządzenie do konkretnej wersji BOM zapisanej na `AssemblyLink`. Wersja BOM ma jawny status lifecycle `ACTIVE`, `INACTIVE` albo `RETIRED`, a wersja `RETIRED` jest traktowana jako niemodyfikowalna. Shipment pozostaje końcową bramką kompletności i korzysta z tej samej wersji, jeśli urządzenie zostało już do niej przypięte; dla nowych, jeszcze nieprzypiętych urządzeń brak aktywnej wersji BOM blokuje przejście dalej. Odczyt `bom-compliance` pokazuje ten sam werdykt na poziomie pojedynczego urządzenia jeszcze przed zmianą statusu shipment, `component-quality` rozpisuje jakość każdego zainstalowanego komponentu z jego snapshotem QC i otwartymi krytycznymi NCR, a na poziomie całego urządzenia wylicza też `passes_component_quality_gate`, `primary_quality_status`, `primary_blocking_component_type`, `primary_blocking_component_serial_number`, `recommended_action`, `device_created_at`, `device_updated_at` i `stale_bucket`. Widok kolejkowy `component-quality` pozwala zebrać te same dane zbiorczo dla wielu urządzeń razem z agregacją per `quality_status`, per `variant_code`, per `production_status`, per `passes_component_quality_gate`, per `primary_quality_status`, per `primary_blocking_component_type`, per `blocking_component_type`, per `stale_bucket`, per `component_type`, per `recommended_action` i z filtrowaniem do konkretnego typu części, wariantu BOM, etapu procesu, werdyktu `passes_component_quality_gate`, dowolnego typu blokującego komponentu, głównego statusu sztuki, głównego typu blockera, konkretnego `primary_blocking_component_serial_number` albo akcji naprawczej. Doszło też filtrowanie po oknach czasu `created_after` / `created_before` oraz `updated_after` / `updated_before`, więc można zawęzić backlog zarówno do nowo utworzonych, jak i ostatnio zmienionych urządzeń, a `staleness_summary` pokazuje rozkład sztuk na przedziały zalegania po `updated_at`, podczas gdy każdy rekord ma już też własny `stale_bucket`. Dodatkowo sam widok można już sortować nie tylko po liczbie blokujących komponentów, ale też po `variant_code`, `recommended_action`, `created_at`, `updated_at`, `passes_component_quality_gate`, `stale_bucket`, `primary_blocking_component_type` i `primary_blocking_component_serial_number`, więc łatwiej grupować urządzenia pod konkretne workflow albo wiek backlogu. `Primary_blocking_component_serial_number` skraca też drogę od kolejki do konkretnej sztuki komponentu wymagającej reworku albo analizy NCR. `Shipment-readiness` dokłada do tego wynik final testu, krytyczne NCR urządzenia oraz blokady jakości komponentów `COMPONENT_QC_NOT_PASSED` i `COMPONENT_CRITICAL_OPEN_NCR`, zwraca rekomendowaną następną akcję oraz snapshot `latest_shipment_gate_decision`, a `coverage` raportuje teraz jawnie także przypadki `OVER_INSTALLED`. Dodatkowo każda próba ustawienia `READY_FOR_SHIPMENT` zapisuje audit `SHIPMENT_GATE_PASSED` albo `SHIPMENT_GATE_BLOCKED`, z pełnym snapshotem wyniku bramki i kodami blokad; ten ślad można odczytać bezpośrednio przez `/api/audit-events` z filtrami `event_type` i `result` albo przez dedykowany endpoint `shipment-gate-history` dla pojedynczego urządzenia. Widok kolejkowy wspiera też filtr `latest_gate_result`, więc można szybko wyłowić sztuki bez żadnej wcześniejszej próby albo tylko te ostatnio blokowane, a także filtr i summary po bieżącym `production_status`, co pomaga rozdzielić urządzenia wg etapu procesu.
