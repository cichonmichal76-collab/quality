...
def downgrade() -> None:
    ...
    dialect_name = op.get_bind().dialect.name
    if dialect_name == "sqlite":
        with op.batch_alter_table("qc_steps") as batch_op:
            batch_op.drop_column("result_input_label")
            batch_op.drop_column("evaluation_mode")
            batch_op.drop_column("control_area")
        ...
        with op.batch_alter_table("qc_checklists") as batch_op:
            batch_op.drop_column("reference_image_file_id")
            batch_op.drop_column("skip_component_qc")
