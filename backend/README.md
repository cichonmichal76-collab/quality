...
Backend udostępnia też odczyt `usage` dla BOM, który pokazuje liczbę powiązanych urządzeń, mutowalność wersji i rekomendowaną kolejną akcję, oraz odczyt `diff`, który porównuje dwie wersje BOM i rozbija zmiany na `added`, `removed`, `modified` i `unchanged_count`.
