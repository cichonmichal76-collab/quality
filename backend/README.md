...
Na poziomie operacyjnym doszło też `GET /api/component-quality`, które zbiera taki sam widok dla wielu urządzeń, wspiera filtry `device_type`, `variant_code`, `production_status`, `quality_status`, `only_blocking`, paginację przez `offset` i `limit`, summary per status jakości komponentu oraz sortowanie `sort_by` / `sort_desc`, domyślnie ustawiające urządzenia z największą liczbą blokujących komponentów na górze.
